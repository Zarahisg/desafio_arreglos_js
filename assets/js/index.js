let inputRooms =document.querySelector('#inputRooms');
...
const propiedadesJSON = [
  ...
  function render(array) {
    if (array.length ===0 ){
      alert('No Hay disponibilidad con sus especificaciones, intenta cambiando los valores ')
...
formulario.addEventListener('submit', (e) => {
  e.preventDefault();
  let cantdCuartos = +inputRooms.value;
  ...
  if (cantdCuartos <= 0 || metrosMin <= 0 || metrosMax < 0) {
    ...
    propiedades.innerHTML = "";
    for (let house of propiedadesJSON) {
      if (house.rooms === cantdCuartos && house.metros >= metrosMin && house.metros <= metrosMax) {
        arrayFiltrado.push(house)
      }
    }
  }
